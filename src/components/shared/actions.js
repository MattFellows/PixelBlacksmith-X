...
export const setPopup = ({popupType, trader, visitor, visitorDemand}) => {
    let action = {
        ...
        trader: trader,
        visitor: visitor,
        visitorDemand: visitorDemand,
    };
    return action
}

const getMultiplier = (visitor, item, state) => {
    const typeMultiplier = (item.type === visitor.visitorType.typePreferred.name) ? visitor.visitorType.typeMultiplier : 1;
    const stateMultiplier = (state === visitor.visitorType.statePreferred.value) ? visitor.visitorType.stateMultiplier : 1;
    const tierMultiplier = (item.tier === visitor.visitorType.tierPreferred.value) ? visitor.visitorType.tierMultiplier : 1;
...
export const sellToVisitor = ({visitor, visitorDemand, item, state}) => {
    ...
    action.saleValue = Math.round(item.baseValue * getMultiplier(visitor, item, state));
